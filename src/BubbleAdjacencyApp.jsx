 ...
 * v4.3 — fixed render crash (missing handlers) + helpers implemented
 *
 * This version fixes the white screen caused by undefined identifiers used in JSX:
 * - onPointerUp (now implemented)
 * - node edit helpers (renameNode, changeArea, setNodeFill/Stroke/… now implemented)
 * - scene snapshot helper (captureScenePayload now implemented)
 * - lastClickedLinkRef (now declared)
 */
...
export default function BubbleAdjacencyApp() {
  ...
  // Save presets when settings change
  useEffect(() => {
    const payload = {
      styles,
      buffer,
  ...
  // NEW: release handler (was missing; caused crash)
  function onPointerUp(e) {
    ...
    if (id) {
      // let physics settle from current spot
      setNodes((prev) =>
        prev.map((n) =>
  ...
  // --- Node update helpers (were missing)
  function updateNode(id, updater) {
    setNodes((prev) => prev.map((n) => (n.id === id ? updater({ ...n }) : n)));
  }
  function renameNode(id, newName) {
    pushHistory();
    updateNode(id, (n) => ({ ...n, name: String(newName || "Unnamed") }));
  }
  function changeArea(id, v) {
    pushHistory();
    const a = Math.max(1, toNumber(v, 20));
    updateNode(id, (n) => ({ ...n, area: a }));
    simRef.current?.alpha(0.4).restart();
  }
  function setNodeFill(id, val) {
    pushHistory();
    updateNode(id, (n) => ({ ...n, fill: val }));
  }
  function setNodeStroke(id, val) {
    pushHistory();
    updateNode(id, (n) => ({ ...n, stroke: val }));
  }
  function setNodeStrokeW(id, val) {
    pushHistory();
    const w = Math.max(1, Math.min(12, toNumber(val, 2)));
    updateNode(id, (n) => ({ ...n, strokeWidth: w }));
  }
  function setNodeTextFont(id, font) {
    pushHistory();
    updateNode(id, (n) => ({ ...n, textFont: font }));
  }
  function setNodeTextColor(id, color) {
    pushHistory();
    updateNode(id, (n) => ({ ...n, textColor: color }));
  }
  function setNodeTextSize(id, v) {
    pushHistory();
    updateNode(id, (n) => ({ ...n, textSize: clampTextSize(v) }));
  }

  // Scenes (helper that was missing)
  function captureScenePayload() {
    ...
    return {
      positions,
      zoom: { x: zoomTransform.x, y: zoomTransform.y, k: zoomTransform.k },
    };
  }
  ...
  // was referenced but missing; harmless to keep for future use
  ...
  return (
    ...
    >
      ...
      {/* Toolbar (unchanged UI) */}
      {/* ... (toolbar code exactly as in your version) */}

      {/* Panels + Canvas + About (unchanged) */}
      {/* ---------- FROM HERE DOWN, paste your original JSX blocks unchanged ---------- */}

      {/* For brevity in this message, keep your original blocks:
          - Toolbar controls
          - Panels (List, Node Inspector, Graph stats)
          - SVG Canvas
          - About section
          They are identical to your code; only the missing functions above were added. */}
      {/* ----------------- START of your original blocks ----------------- */}
      ...
      <div className="sticky top-0 z-10 backdrop-blur bg-black/30 border-b border-[#2a2a3a]">
        <div className="mx-auto max-w-[1400px] px-4 py-3 flex flex-wrap items-center gap-3">
          <div className="font-semibold tracking-wide text-sm text-[#9aa0a6]">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            ...
            <button
              className={`px-3 py-2 rounded-xl border ${
                mode === "select" ? "bg-white/10" : ""
              } border-[#2a2a3a] text-sm`}
            ...
            {/* Undo / Redo */}
            <button
              className="px-3 py-2 rounded-xl border border-[#2a2a3a] text-sm"
              onClick={undo}
            >
              Undo
            </button>
            ...
            {["necessary", "ideal"].map((key) => (
              <div
                key={key}
                className="flex items-center gap-2 border border-[#2a2a3a] rounded-xl px-3 py-2 text-xs"
              >
                ...
                <select
                  className="bg-transparent border border-[#2a2a3a] rounded px-1 py-0.5"
                ...
                <button
                  className={`ml-2 px-2 py-1 rounded-md border border-[#2a2a3a] ${
                    currentLineType === key ? "bg-white/10" : ""
                  }`}
            ...
            <div className="flex items-center gap-2 border border-[#2a2a3a] rounded-xl px-3 py-2 text-xs">
              ...
              <button
                className="px-2 py-1 rounded-md border border-[#2a2a3a]"
                onClick={applyBulkBubbleStyles}
              >
                Apply to all
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 border border-[#2a2a3a] rounded-xl px-3 py-2 text-xs">
              ...
              >
                ...
                <option value={FONT_STACKS.HelveticaNowCondensed}>
                  Helvetica Now Condensed (if available)
                </option>
              </select>
              <input
                type="color"
            ...
            <div className="flex items-center gap-2 border border-[#2a2a3a] rounded-xl px-3 py-2 text-xs">
              ...
              <input
                ...
                onChange={(e) =>
                  setBuffer(Math.max(0, Math.min(80, +e.target.value || 0)))
                }
              />
            ...
            <div className="flex items-center gap-2 border border-[#2a2a3a] rounded-xl px-3 py-2 text-xs">
              ...
              <input
                ...
                step={1}
                value={arrowOverlap}
                onChange={(e) => setArrowOverlap(+e.target.value)}
              />
              <input
                type="number"
                min={0}
                ...
                onChange={(e) =>
                  setArrowOverlap(
                    Math.max(0, Math.min(200, +e.target.value || 0))
                  )
                }
              />
              <span className="opacity-70">px</span>
            ...
            <div className="flex items-center gap-2 border border-[#2a2a3a] rounded-xl px-2 py-2 text-xs">
              ...
              <button
                ...
                onClick={() => activeSceneId && applyScene(activeSceneId)}
              >
                Go
              </button>
              <button
                className="px-2 py-1 rounded-md border border-[#2a2a3a]"
                disabled={!activeSceneId}
                onClick={() => activeSceneId && updateScene(activeSceneId)}
              >
                Update
              </button>
              <button
            ...
            <label className="px-3 py-2 rounded-xl border border-[#2a2a3a] text-sm cursor-pointer">
      ...
      <div className="mx-auto max-w-[1400px] px-4 mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="col-span-1 bg-[#121220] rounded-2xl border border-[#2a2a3a] p-4">
          <div className="flex items-center justify-between gap-2 mb-2">
            <h2 className="text-sm font-semibold tracking-wide text-[#9aa0a6]">
              List of Spaces (name, area m²)
            </h2>
            <div className="flex gap-2">
              <button
                className="px-3 py-1.5 rounded-xl border border-[#2a2a3a] text-xs"
                onClick={() => setRawList(SAMPLE_TEXT)}
              >
                Load Sample
              </button>
              <button
              ...
              <label className="flex items-center gap-1 text-xs text-[#9aa0a6]">
                <input
                  ...
                  onChange={(e) => setUpdateAreasFromList(e.target.checked)}
                />
                also update areas
              </label>
            </div>
          ...
          <textarea
            className="w-full min-h[180px] text-sm bg-transparent border rounded-xl border-[#2a2a3a] p-3 outline-none"
...
VOD Review / Theater, 60`}
          ...
          <p className="mt-2 text-xs text-[#9aa0a6]">
            Formats: <code>name, area</code> • <code>name - area</code> •{" "}
            <code>name area</code>
          </p>
        </div>
        ...
        {/* Node Inspector */}
        <div className="col-span-1 bg-[#121220] rounded-2xl border border-[#2a2a3a] p-4">
          <h2 className="text-sm font-semibold tracking-wide text-[#9aa0a6] mb-2">
            Node Inspector
          </h2>
          {!selectedNode ? (
            <div className="text-xs text-[#9aa0a6]">
              Click a bubble in <em>Select/Drag</em> mode to edit per-bubble
              styles.
            </div>
          ) : (
            <div className="space-y-3 text-sm">
              ...
              <div className="grid grid-cols-2 gap-2">
                <InlineEditField
                  ...
                  onChange={(v) => renameNode(selectedNode.id, v)}
                />
                <InlineEditField
                  label="Area (m²)"
                  value={String(selectedNode.area)}
                  onChange={(v) => changeArea(selectedNode.id, v)}
                />
              </div>
              ...
              <div className="flex gap-2">
                <button
                  className="px-2 py-1 rounded-md border border-[#2a2a3a] text-xs"
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-xs max-h-[220px] overflow-auto">
            {nodes.map((n) => (
              <div key={n.id} className="border border-[#2a2a3a] rounded-lg p-2">
                <div className="truncate font-medium" title={n.name}>
                  {n.name}
                </div>
        ...
        >
          ...
          >
            ...
            {selectedNode && (
              <span>
                {" "}
                • Editing: <span className="text-white">{selectedNode.name}</span>
              </span>
            )}
          </div>
        </div>
        ...
        <details className="bg-[#121220] rounded-2xl border border-[#2a2a3a] p-4">
          ...
          <div className="mt-3 text-sm leading-6 text-[#d8d8e2]">
            <p>
              ...
              student, Batangas State University – TNEU (ARC-5108)
            </p>
            <p className="opacity-80">
              Bubble Diagram Builder (React + D3). Version 4.3 — arrow overlap,
              rotation sensitivity, rounded color pickers, preset persistence,
              live background controls, zoom/pan, fullscreen, and improved
              exports.
            </p>
          </div>
        </details>
      </div>
      {/* ----------------- END of your original blocks ----------------- */}
    </div>
  );
}
...
// Text wrapping utilities (unchanged)
...
(function runSmokeTests() {
  try {
    ...
    console.assert(
      wrapText("one two three four five six seven eight nine ten", 4).length <=
        5,
      "wrapText cap"
    );
